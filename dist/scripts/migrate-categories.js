...
async function main() {
    ...
    `);
        ...
        for (const cat of DEFAULT_CATEGORIES) {
            ...
            const existingResult = await client.query('SELECT id FROM categories WHERE name = $1', [cat.name]);
            if (existingResult.rows.length > 0) {
                categoryIdMap.set(cat.name, existingResult.rows[0].id);
                console.log(`   ⏭️  Skipped ${cat.name} (already exists)`);
            }
            else {
                await client.query(`INSERT INTO categories (id, name, "displayName", "sortOrder", "isActive", "createdAt", "updatedAt")
           VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`, [categoryId, cat.name, cat.displayName, cat.sortOrder, true]);
    ...
    `);
        if (oldColumnCheck.rows.length > 0) {
            ...
            const menuItems = await client.query('SELECT id, category FROM menu_items WHERE "categoryId" IS NULL');
            let updatedCount = 0;
            for (const item of menuItems.rows) {
                const categoryId = categoryIdMap.get(item.category);
                if (categoryId) {
                    await client.query('UPDATE menu_items SET "categoryId" = $1 WHERE id = $2', [categoryId, item.id]);
                    updatedCount++;
                }
            }
        ...
        const nullCheck = await client.query('SELECT COUNT(*) FROM menu_items WHERE "categoryId" IS NULL');
        if (parseInt(nullCheck.rows[0].count) > 0) {
            ...
            await client.query('UPDATE menu_items SET "categoryId" = $1 WHERE "categoryId" IS NULL', [firstCategory]);
