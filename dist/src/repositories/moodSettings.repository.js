...
export class MoodSettingsRepository {
    ...
    async getMoodSettingByMood(mood) {
        return this.prisma.mood_settings.findUnique({
            where: { mood }
        });
    }
    async updateMoodSetting(mood, data) {
        const updateData = {};
        ...
        if (data.supportMessage !== undefined)
            updateData.supportMessage = data.supportMessage;
        if (data.scientificExplanation !== undefined)
            updateData.scientificExplanation = data.scientificExplanation;
        if (data.beneficialNutrients !== undefined)
            updateData.beneficialNutrients = JSON.stringify(data.beneficialNutrients);
        if (data.preferredCategories !== undefined)
            updateData.preferredCategories = JSON.stringify(data.preferredCategories);
        if (data.excludeCategories !== undefined)
            updateData.excludeCategories = JSON.stringify(data.excludeCategories);
    ...
    async incrementMoodOrdered(mood) {
    ...
    async recordMoodFeedback(mood, outcome, orderId) {
    ...
    async resetMoodOrderStats(mood) {
        if (mood) {
    ...
    async incrementItemsShown(menuItemIds, mood) {
        const updates = menuItemIds.map(menuItemId => this.prisma.menu_item_mood_stats.upsert({
    ...
    async incrementItemsOrdered(menuItemIds, mood) {
        const updates = menuItemIds.map(menuItemId => this.prisma.menu_item_mood_stats.upsert({
            where: { menuItemId_mood: { menuItemId, mood } },
            create: {
                id: randomUUID(),
    ...
    async recordItemsFeedback(menuItemIds, mood, outcome) {
        const incrementData = {
            feedbackCount: { increment: 1 },
            updatedAt: new Date()
        };
    ...
    async getItemMoodStats(menuItemId) {
    ...
    async getTopItemsForMood(mood, limit = 10) {
    ...
    async getDetailedMoodAnalytics(mood) {
    ...
    async resetMoodOrderStatsByMood(mood) {
        await this.prisma.mood_order_stats.update({
            where: { mood },
            data: {
                totalShown: 0,
    ...
    async resetMenuItemMoodStatsByMood(mood) {
        await this.prisma.menu_item_mood_stats.deleteMany({
            where: { mood }
        });
    ...
    async resetMenuItemMoodStatsByItem(menuItemId) {
        await this.prisma.menu_item_mood_stats.deleteMany({
            where: { menuItemId }
        });
