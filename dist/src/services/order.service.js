...
export class OrderService {
     ...
     */
    async voidOrder(orderId, params) {
     ...
     */
    async replenishInventoryForVoidedOrder(orderId, reason) {
        ...
        for (const [_, requirement] of inventoryRequirements) {
            try {
                ...
                console.log(`✓ Replenished ${requirement.totalRequired} ${requirement.unit} of ${requirement.inventoryItemName} for voided order ${order.orderNumber}`);
            }
            catch (error) {
                console.error(`✗ Failed to replenish ${requirement.inventoryItemName}: ${error.message}`);
            }
        }
        console.log(`Stock replenishment completed for voided order ${order.orderNumber}`);
    }
    /**
