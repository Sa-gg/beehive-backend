...
async function backupDatabase() {
  ...
  try {
    ...
    // Backup mood_settings
    console.log('📦 Backing up mood_settings...');
    const moodSettings = await prisma.mood_settings.findMany();
    backup.mood_settings = moodSettings;
    console.log(`   ✅ ${moodSettings.length} mood settings`);
    
    // Backup mood_feedback_config
    console.log('📦 Backing up mood_feedback_config...');
    const moodFeedbackConfig = await prisma.mood_feedback_config.findMany();
    backup.mood_feedback_config = moodFeedbackConfig;
    console.log(`   ✅ ${moodFeedbackConfig.length} mood feedback configs`);
    
    // Backup mood_order_stats
    console.log('📦 Backing up mood_order_stats...');
    const moodOrderStats = await prisma.mood_order_stats.findMany();
    backup.mood_order_stats = moodOrderStats;
    console.log(`   ✅ ${moodOrderStats.length} mood order stats`);
    
    // Backup menu_item_mood_stats
    console.log('📦 Backing up menu_item_mood_stats...');
    const menuItemMoodStats = await prisma.menu_item_mood_stats.findMany();
    backup.menu_item_mood_stats = menuItemMoodStats;
    console.log(`   ✅ ${menuItemMoodStats.length} menu item mood stats`);
